 ...
 *
 * This only runs when "Режим оплати" is set to "API" on the settings
 * page. In the default "Пряме посилання" mode the CTA button stays a
 * plain link to vip_tattoo_plan_stripe_url and none of this fires --
 * js/checkout.js checks window.VIP_TATTOO_PLAN_CHECKOUT_MODE before
...
function vip_tattoo_plan_payment_fields() {
    return [
        // 'link' = CTA is a plain <a href> to vip_tattoo_plan_stripe_url
        // (the simple mode, no keys needed). 'api' = CTA opens the
        // email/phone popup and goes through the checkout below.
        'vip_tattoo_plan_checkout_mode'              => 'link',
        ...
        'vip_tattoo_plan_telegram_access_message'      => "Оплата прошла успешно! 🎉\n\nВот твой доступ к курсу VIP tattoo school:\n[вставь сюда ссылку/данные для входа]\n\nЕсли есть вопросы — просто напиши сюда, в этот чат.",
    ];
}
    ...
    <div class="wrap">
        ...
        <form method="post">
            ...
            <table class="form-table">
                <tr>
                    <th><label>Як працює кнопка «<?php echo esc_html(get_option('vip_tattoo_plan_cta_text', 'Открыть доступ к обучению')); ?>»</label></th>
                    <td>
                        <label><input type="radio" name="vip_tattoo_plan_checkout_mode" value="link" <?php checked($vals['vip_tattoo_plan_checkout_mode'], 'link'); ?> /> Пряме посилання (готовий Stripe Payment Link, без ключів нижче — поточний спосіб)</label><br />
                        <label><input type="radio" name="vip_tattoo_plan_checkout_mode" value="api" <?php checked($vals['vip_tattoo_plan_checkout_mode'], 'api'); ?> /> API-оплата (Stripe/PayPal нижче, з тестовими та live ключами, вебхуками і доставкою доступу в Telegram)</label>
                        <p class="description">У режимі «Пряме посилання» все нижче на цій сторінці не використовується — кнопка просто веде на посилання з розділу «Оплата і кнопки» основних налаштувань.</p>
                    </td>
                </tr>
                <tr>
                    <th><label>Активний провайдер (тільки для режиму API)</label></th>
                    <td>
                        ...
                        <p class="description">Налаштування іншого провайдера нижче нікуди не зникають — можна перемкнутись назад у будь-який момент.</p>
                    </td>
                </tr>
                <tr>
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description">Якщо заповнено — оплата йде саме на цю ціну з каталогу Stripe. Якщо порожньо — плагін сам створює одноразовий товар на льоту з полів «Ціна»/«Валюта»/«Назва товару» вище.</p>
                    </td>
                </tr>
            </table>
            ...
            <h2>Telegram-доступ (надсилається після оплати, режим API)</h2>
            <table class="form-table">
                <tr>
        ...
        <hr />
        <h2>Останні замовлення (режим API)</h2>
...
function vip_tattoo_plan_render_recent_orders() {
    ...
    echo '<table class="widefat striped"><thead><tr><th>Email</th><th>Телефон</th><th>Провайдер</th><th>Статус</th><th>Telegram</th><th>Створено</th><th>Оплачено</th></tr></thead><tbody>';
    foreach ($rows as $row) {
        printf(
            ...
            esc_html($row->status),
            $row->delivered_at ? '✓ доставлено' : '—',
            esc_html($row->created_at),
            esc_html($row->paid_at ?: '—')
        );
    }
    echo '</tbody></table>';
}
...
function vip_tattoo_plan_checkout_mode() {
    $mode = get_option('vip_tattoo_plan_checkout_mode', 'link');
    return $mode === 'api' ? 'api' : 'link';
}
...
function vip_tattoo_plan_rest_create_checkout(WP_REST_Request $request) {
    ...
    if (vip_tattoo_plan_checkout_mode() !== 'api') {
        return new WP_REST_Response(['error' => 'API-оплата вимкнена в налаштуваннях.'], 400);
    }

    $email = sanitize_email($request->get_param('email'));
    $phone = sanitize_text_field($request->get_param('phone'));
    if (!$email) {
        return new WP_REST_Response(['error' => "Email обов'язковий."], 400);
    }
